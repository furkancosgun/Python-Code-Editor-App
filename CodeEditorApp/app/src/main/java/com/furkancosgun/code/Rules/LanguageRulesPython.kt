package com.furkancosgun.code.Rules

import com.furkancosgun.code.Model.RuleMatch
...
class LanguageRulesPython : LanguageRules {
    ...
    override fun matchKeywords(string: CharSequence): List<RuleMatch> {
        val match: Regex =
            "\\b(return|print|pass|lambda|with|is|not|in|from|elif|raise|del)(?=\\b)|(def)\\s+(\\w+)(?=\\()".toRegex()
        return match.findAll(string).map {
